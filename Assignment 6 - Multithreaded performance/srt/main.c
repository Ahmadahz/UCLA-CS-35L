...
/* We make nthreads global so we can use it in our perform_work function */
int nthreads;

/* we make scene global for the same reason as above */
scene_t scene;
 
/* global array to output pixels */
float pixelOutput[width][height][3];

//we put the individual pixel work in a function so we can divide
//the work among the threads
void* perform_work(void *thread_args)
{
    Vec3 camera_pos;
    ...
    /* for every pixel */
    for( int px = *(int*) thread_args ; px<width; px += nthreads)
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            //instead of printing in the function, we store the output in our
            //pixelOutput array, and we print all pixels at the end
            for (int i = 0; i < 3; i++)
            {
                pixelOutput[px][py][i] = scaled_color[i];
            }
        }
    }   //end for loop
    return NULL;
...
int main( int argc, char **argv )
{
    ...
    for (i = 0; i < nthreads; i++)
    {
        ...
        if (result_code != 0)
        {
            fprintf(stderr, "Error Code %d: Could not create thread %d.\n", result_code, i);
            exit(result_code);
        }
    }
    ...
    for (i = 0; i < nthreads; i++)
    {
        ...
        if (result_code != 0)
        {
            fprintf(stderr, "Error Code %d: Could not join thread %d.\n", result_code, i);
            exit(result_code);
        }
    }
    ...
    for (int x = 0; x < width; x++)
    {
        ...
        {
            printf( "%.0f %.0f %.0f\n", pixelOutput[x][y][0], pixelOutput[x][y][1], pixelOutput[x][y][2]);
        }
        printf("\n");
    }
    
   //this is where the code was that I copied and pasted into the perform_work function
